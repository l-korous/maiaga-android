...
long TinyGPSPlus::parseDecimal(const char *term)
{
  bool negative = *term == '-';
  if (negative) ++term;
  long ret = 100 * (long)atol(term);
...
{
  long leftOfDecimal = (long)atol(term);
  int minutes = (int)(leftOfDecimal % 100);
  long multiplier = 10000000UL;
  long tenMillionthsOfMinutes = minutes * multiplier;
...
{
   newTime = (long)TinyGPSPlus::parseDecimal(term);
}
